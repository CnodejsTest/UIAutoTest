...
Then(/^登录状态"([^"]*)"，提示信息"([^"]*)"$/, async function(status, infomessage) {
  ...
  switch (status) {
    ...
    case '成功':
      ...
      let loginname_text = await driver
        .findElement({ css: '#sidebar > div:nth-child(1) > div.inner > div > div > span.user_name' })
        .getText()
      return assert.deepEqual(loginname_text, infomessage)
  }
})

Then(/^退出登陆$/, async function () {
    let sign_out = await driver.findElement({css: 'a[href="/signout"]'})
    await sign_out.click()
});

Given(/^浏览到网站首页\(一\)$/,async function () {
    await driver.get(rootUrl);
});

When(/^点击登录按钮应该跳转到登录页面\(二\)$/, async function () {
    let login_link = await driver.findElement({css: 'a[href="/signin"]'});
    await login_link.click();
    let current_url = await driver.getCurrentUrl();
    return assert.ok(current_url, rootUrl+'/signin');
});

When(/^账号栏输入"([^"]*)"$/, async function (username) {
    let username_field = await driver.findElement({id: 'name'})
    await username_field.sendKeys(username);
});

When(/^密码栏输入"([^"]*)"$/, async function (passwd) {
    let passwd_field = await driver.findElement({id: 'pass'})
    await passwd_field.sendKeys(passwd)
});

When(/^点击登陆按钮$/, async function () {
    let login_btn = await driver.findElement({ css: 'input[type="submit"]' })
    await login_btn.click()
});

Then(/^登陆状态“(\d+)”，提示信息"([^"]*)"$/, async function (status, infomessage) {
    let current_url = await driver.getCurrentUrl()
    switch (status) {
        case '失败':
            return assert.equal(current_url, rootUrl+'/signin')
        case '成功':
            return assert.equal(current_url, rootUrl)
    }
});
